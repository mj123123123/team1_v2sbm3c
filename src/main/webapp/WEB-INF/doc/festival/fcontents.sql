...
/**********************************/
DROP TABLE fcontents CASCADE CONSTRAINTS; -- 자식 무시하고 삭제 가능
CREATE TABLE fcontents(
        fcontentsno                            NUMBER(10)         NOT NULL         PRIMARY KEY,
...
COMMENT ON TABLE fcontents is '컨텐츠 - 순례길';
COMMENT ON COLUMN fcontents.fcontentsno is '컨텐츠 번호';
COMMENT ON COLUMN fcontents.adminno is '관리자 번호';
COMMENT ON COLUMN fcontents.fcateno is '카테고리 번호';
...
COMMENT ON COLUMN fcontents.file1saved is '실제 저장된 파일명';
...
COMMENT ON COLUMN fcontents.youtube is 'Youtube 영상';
...
INSERT INTO fcontents(fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, passwd, 
                     word, rdate, file1, file1saved, thumb1, size1)
VALUES(fcontents_seq.nextval, 1, 1, '오버랜딩', '4WD 준비', 0, 0, 0, '123',
       '자연', sysdate, 'space.jpg', 'space_1.jpg', 'space_t.jpg', 1000);

-- 유형 1, 전체 목록
SELECT fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, passwd, word, rdate,
           file1, file1saved, thumb1, size1
FROM fcontents
ORDER BY fcontentsno DESC;

-- 유형 2, 카테고리별 목록
INSERT INTO fcontents(fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, passwd, 
                     word, rdate, file1, file1saved, thumb1, size1)
VALUES(fcontents_seq.nextval, 1, 2, '태백', '산악 주행', 0, 0, 0, '123',
       '드라마,K드라마,넷플릭스', sysdate, 'space.jpg', 'space_1.jpg', 'space_t.jpg', 1000);
            
INSERT INTO fcontents(fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, passwd, 
                     word, rdate, file1, file1saved, thumb1, size1)
VALUES(fcontents_seq.nextval, 1, 2, '속초', '비포장 임도', 0, 0, 0, '123',
       '드라마,K드라마,넷플릭스', sysdate, 'space.jpg', 'space_1.jpg', 'space_t.jpg', 1000);

INSERT INTO fcontents(fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, passwd, 
                     word, rdate, file1, file1saved, thumb1, size1)
VALUES(fcontents_seq.nextval, 1, 2, '홍천', '서울에서 가까운 지역', 0, 0, 0, '123',
       '드라마,K드라마,넷플릭스', sysdate, 'space.jpg', 'space_1.jpg', 'space_t.jpg', 1000);
...
-- 삭제
DELETE FROM fcontents
WHERE fcontentsno = 25;
commit;

DELETE FROM fcontents
WHERE fcateno=12 AND fcontentsno <= 41;
...
-- ----------------------------------------------------------------------------------------------------
-- 모든글
SELECT fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, word, rdate,
       file1, file1saved, thumb1, size1, map, youtube
FROM fcontents
ORDER BY fcontentsno ASC;
...
SELECT fcontentsno || '. ' || title || ' 태그: ' || word as title -- 컬럼의 결합, ||
FROM fcontents
WHERE fcateno=1 AND (LOWER(word) LIKE '%' || LOWER('Food') || '%'); -- 대소문자를 일치 시켜서 검색
...
-- ----------------------------------------------------------------------------------------------------
-- step 1
SELECT fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, rdate,
           file1, file1saved, thumb1, size1, map, youtube
FROM fcontents
...
-- ----------------------------------------------------------------------------
-- MAP 등록/수정
UPDATE fcontents SET map='카페산 지도 스크립트' WHERE fcontentsno=1;

-- MAP 삭제
UPDATE fcontents SET map='' WHERE fcontentsno=1;
...
-- ----------------------------------------------------------------------------
-- youtube 등록/수정
UPDATE fcontents SET youtube='Youtube 스크립트' WHERE fcontentsno=1;

-- youtube 삭제
UPDATE fcontents SET youtube='' WHERE fcontentsno=1;
...
SELECT COUNT(*) as cnt 
FROM fcontents
WHERE fcontentsno=1 AND passwd='123';
...
SELECT fcontentsno, adminno, fcateno, title
FROM fcontents
WHERE fcateno IN(1,2,3);

fcontentsNO    ADMINNO     fcateNO TITLE                                                                                                                                                                                                                                                                                                       
---------- ---------- ---------- ------------------------
         3             1                   1           인터스텔라                                                                                                                                                                                                                                                                                                  
         4             1                   2           드라마                                                                                                                                                                                                                                                                                                      
         5             1                   3           컨저링                                                                                                                                                                                                                                                                                                      
...
-- ----------------------------------------------------------------------------------------------------
-- fcate + fcontents INNER JOIN
-- ----------------------------------------------------------------------------------------------------
-- 모든글
SELECT c.name,
       t.fcontentsno, t.adminno, t.fcateno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
       t.file1, t.file1saved, t.thumb1, t.size1, t.map, t.youtube
FROM fcate c, fcontents t
WHERE c.fcateno = t.fcateno
ORDER BY t.fcontentsno DESC;

-- fcontents, admin INNER JOIN
SELECT t.fcontentsno, t.adminno, t.fcateno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
       t.file1, t.file1saved, t.thumb1, t.size1, t.map, t.youtube,
       a.mname
FROM admin a, fcontents t
...
SELECT t.fcontentsno, t.adminno, t.fcateno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
       t.file1, t.file1saved, t.thumb1, t.size1, t.map, t.youtube,
       a.mname
FROM admin a INNER JOIN fcontents t ON a.adminno = t.adminno
ORDER BY t.fcontentsno DESC;

-- ----------------------------------------------------------------------------------------------------
-- View + paging
-- ----------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW vfcontents
AS
SELECT fcontentsno, adminno, fcateno, title, content, recom, cnt, replycnt, word, rdate,
        file1, file1saved, thumb1, size1, map, youtube
FROM fcontents
...
-- ----------------------------------------------------------------------------------------------------
SELECT fcontentsno, adminno, fcateno, title, thumb1, r
FROM (
           SELECT fcontentsno, adminno, fcateno, title, thumb1, rownum as r
           FROM (
                     SELECT fcontentsno, adminno, fcateno, title, thumb1
                     FROM fcontents
                     WHERE fcateno=1
                     ORDER BY recom DESC
           )          
