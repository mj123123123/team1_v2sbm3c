...
CREATE TABLE FESTIVAL(
  ...
  FOREIGN KEY (festivalcateno) REFERENCES festivalcate (festivalcateno)
);

--COMMENT ON TABLE FESTIVAL is '축제/행사 게시물';
COMMENT ON COLUMN FESTIVAL.festivalno IS '축제 게시물 번호';
COMMENT ON COLUMN FESTIVAL.festivalcateno IS '축제 카테고리 번호';
...
INSERT INTO festival (
    festivalno, festivalcateno, festivaltitle, festivalcontent, festivallikecnt,
...
-- 조회
SELECT festivalno, festivalcateno, festivaltitle, festivalcontent,
...
DELETE FROM festival
WHERE festivalcateno = 12 AND festivalno <= 41;
COMMIT;
...
-- ----------------------------------------------------------------------------------------------------
-- 모든글
SELECT festivalno, festivalcateno, festivaltitle, festivalcontent, festivallikecnt,
       festivalcnt, pwd, festivalword, festivalrdate, festivaludate,
       festivalfile1, festivalfile1saved, festivalthumb1, festivalsize1, festivalmap, festivalyoutube
FROM FESTIVAL
...
SELECT festivalno, festivalcateno, festivaltitle, festivalcontent, festivallikecnt,
       ...
       festivalfile1, festivalfile1saved, festivalthumb1, festivalsize1, festivalmap, festivalyoutube
FROM FESTIVAL
WHERE festivalcateno = 2 AND festivalword LIKE '%꽃%'
ORDER BY festivalno DESC;
...
SELECT festivalno, festivalcateno, festivaltitle, festivalcontent, festivallikecnt,
       ...
       festivalfile1, festivalfile1saved, festivalthumb1, festivalsize1, festivalmap, festivalyoutube
FROM FESTIVAL
WHERE festivalcateno = 2 AND (festivaltitle LIKE '%꽃%' OR festivalcontent LIKE '%꽃%' OR festivalword LIKE '%꽃%')
ORDER BY festivalno DESC;
...
FROM FESTIVAL
WHERE festivalcateno = 2 AND (festivalcontent LIKE '%꽃%');

-- SQL은 대소문자를 구분하지 않으나 WHERE문에 명시하는 값은 대소문자를 구분하여 검색
SELECT festivalno, festivaltitle, festivalword
FROM FESTIVAL
WHERE festivalcateno = 1 AND (festivalword LIKE '%FOOD%');
...
SELECT festivalno, festivaltitle, festivalword
FROM FESTIVAL
WHERE festivalcateno = 1 AND (LOWER(festivalword) LIKE '%food%'); -- 대소문자를 일치 시켜서 검색

-- ||: 문자열 연결
-- LIKE '%' || UPPER('FOOD') || '%' -> LIKE '%FOOD%'
SELECT festivalno, festivaltitle, festivalword
FROM FESTIVAL
WHERE festivalcateno = 1 AND (UPPER(festivalword) LIKE '%' || UPPER('FOOD') || '%'); -- 대소문자를 일치 시켜서 검색 ★

SELECT festivalno, festivaltitle, festivalword
FROM FESTIVAL
WHERE festivalcateno = 1 AND (LOWER(festivalword) LIKE '%' || LOWER('Food') || '%'); -- 대소문자를 일치 시켜서 검색
...
FROM (
           ...
           FROM (
                     SELECT festivalno, festivalcateno, festivaltitle, festivalcontent, festivallikecnt,
                            ...
                            festivalfile1, festivalfile1saved, festivalthumb1, festivalsize1, festivalmap, festivalyoutube
                     FROM FESTIVAL
                     WHERE festivalcateno = 2 AND (UPPER(festivaltitle) LIKE '%' || UPPER('꽃') || '%' 
...
-- festivalcateno FK 특정 그룹에 속한 레코드 갯수 산출
SELECT COUNT(*) as cnt 
FROM festival 
WHERE festivalcateno=7;

-- festivalcateno FK 특정 그룹에 속한 레코드 모두 삭제
DELETE FROM festival
WHERE festivalcateno=7;
...
SELECT COUNT(*) as cnt
FROM festival
WHERE festivalcateno IN(1,2,3);

-- 다수의 카테고리에 속한 레코드 모두 SELECT
SELECT festivalno, festivalcateno, festivaltitle
FROM festival
WHERE festivalcateno IN(1,2,3);

-- FK 컬럼별 삭제: 1번 카테고리 관련글 모두 삭제 
DELETE FROM festival
WHERE festivalcateno=5;
...
-- 모든 글
SELECT c.catename,
       f.festivalno, f.festivalcateno, f.festivaltitle, f.festivalcontent, f.festivallikecnt, 
...
FROM festivalcate c
JOIN festival f ON c.festivalcateno = f.festivalcateno
...
FROM (
     SELECT festivalno, festivalcateno, festivaltitle, festivalcontent, festivallikecnt, 
            ...
            rownum as r
     FROM vfestival -- View
     WHERE festivalcateno=14 AND (festivaltitle LIKE '%꽃%' OR festivalcontent LIKE '%꽃%' OR festivalword LIKE '%꽃%')
)
WHERE r >= 1 AND r <= 3;
...
-- ----------------------------------------------------------------------------------------------------
SELECT festivalno, festivalcateno, festivaltitle, festivalthumb1, r
FROM (
           SELECT festivalno, festivalcateno, festivaltitle, festivalthumb1, rownum as r
           FROM (
                     SELECT festivalno, festivalcateno, festivaltitle, festivalthumb1
                     FROM festival
                     WHERE festivalcateno=1
